...
# print(data[int(index_of_last_catsel9_candidate)+1:900])
# print(data[870:int(index_of_last_catsel9_candidate)+1])
print("\n----------------------------------------------------------------------------\n")
...
# Then do allocation of all candidates till the last candidate selected on UR seat
# for example if the sr.no. of last candidate selected on UR seat is 893, then do allocation all candidates from sr.no. 1 to 893

print("\n\n##################################################################################################")
print("      Allocation of candidates till last selected UR Seat, i.e. Sr. no.: ", index_of_last_catsel9_candidate, ', AIR: ', air_of_last_catsel9_candidate)
print("##################################################################################################")

for row in data[1:int(index_of_last_catsel9_candidate)+1]:
    ...
    # use form filled flag to do allocation for only those who have filled form
    if row[24] and (row[6] == '9' or row[6] == '0' or row[6] == '1' or row[6] == '2' or row[6] == '6'):
        print('\n\n--------------------------------------- \n')
        print("Name: ", row[1])
        print("AIR: ", row[7])
        print("Cat 1:", cat[row[3]])
        ...
        # for all (cat1 == catsel) till last candidate selected on UR Seat
        if row[3] == row[6]:
            for i in pref:
                tracker = tracker + 1
                if zone_vacancy[i]['vac'][cat[row[6]]] > 0:
                    ...
                    # now reduce vacancy position of alloted zone for particular category
                    zone_vacancy[i]['vac'][cat[row[6]]] = zone_vacancy[i]['vac'][cat[row[6]]] - 1
                    ...
                    print(' \n ======================================================================== [loop2=>cat1=catsel=>till_last_cand_sel_on_UR_seat]\n')
                    break

        # for EWS, OBC, SC, ST on UR (cat1 != catsel && catsel == 9) till last candidate selected on UR seat
        else:
...
# retry allocation for 1 to 893 with unfilled ur seats
# for row in data[1:int(index_of_last_catsel9_candidate)+1]:
...
# then perform separately for 894:rest
for row in data[int(index_of_last_catsel9_candidate)+1:]:
...
# Now generate cutoffs for each zone
for row in data[1:]:
    # check if candidate filled form
    if row[24]:
        air = row[7]
        zone_code = row[27]
        allocated_cat = row[28]
        if allocated_cat == 'UR':
            zone_vacancy[zone_code]['cutoff']["UR_cutoff"] = air
        if allocated_cat == 'OH':
            zone_vacancy[zone_code]['cutoff']["OH_cutoff"] = air
        if allocated_cat == 'HH':
            zone_vacancy[zone_code]['cutoff']["HH_cutoff"] = air
        if allocated_cat == 'EWS' or allocated_cat == 'EWS PROVISIONAL':
            zone_vacancy[zone_code]['cutoff']["EWS_cutoff"] = air
        if allocated_cat == 'OBC' or allocated_cat == 'OBC PROVISIONAL':
            zone_vacancy[zone_code]['cutoff']["OBC_cutoff"] = air
        if allocated_cat == 'SC' or allocated_cat == 'SC PROVISIONAL':
            zone_vacancy[zone_code]['cutoff']["SC_cutoff"] = air
        if allocated_cat == 'ST'or allocated_cat == 'ST PROVISIONAL':
            zone_vacancy[zone_code]['cutoff']["ST_cutoff"] = air
...
writefilename = 'cgst_zone_allocation_v3.csv'
file = open(writefilename, 'w+', newline ='') 
with file:     
    write = csv.writer(file) 
    write.writerows(data) 

writefilename = 'cgst_zone_cutoff_v3.csv'
file = open(writefilename, 'w+', newline ='') 
with file:     
    write = csv.writer(file) 
    write.writerows(cutoff_tuples) 